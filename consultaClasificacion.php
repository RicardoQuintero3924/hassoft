...
if (isset($_POST['buscarFecha'])) {
    ...
    if (empty($_POST['fecha'])) {
        ...
        $clasificaciones = $controlClasificacion->consultaCategoriaPorFecha(date("F j, Y", strtotime($_POST['fecha'])));
    }
}
    ...
    <div class="separacion">
        <table class="tabla">
            <div style="position: relative; text-align: center;">
                <h2 style="display: inline-block" id="titulo">Consultar cosechas</h2>
                <div style="position: absolute; top: -30px; right: 130px;">
                    <form class="form-inline my-2 my-lg-0" id="form" method="POST" style="text-align: right; margin-top: 25px !important;">
                        ...
                        <input type="submit" value="Buscar" name="buscarFecha">
                    </form>
                </div>
            </div>
            <tr class="celdas">
                <th>Orden</th>
                <th>Número de aguacates</th>
                <th>Peso total</th>
                <th>Fecha inicial</th>
                <th>Fecha final</th>
                <th>Ver cosecha</th>
            ...
            <?php foreach ($clasificaciones as $key=>$clasificacion) :?>
                <tr class="filas">
                    <td>Cosecha<?= $key + 1 ?></td>
                    <td><?= $clasificacion->nro_aguacates ?></td>
                    <td><?= $clasificacion->peso_total?></td>
                    <td><?= $clasificacion->fecha_inicial?></td>
                    <td><?= $clasificacion->fecha_final?></td>
                    <td><a href="clasificacionDetalle.php?cod_clasificacion=<?= $clasificacion->cod_clasificacion ?>" class="btn-table">Ver</a></td>
                </tr>
            <?php endforeach; ?>
